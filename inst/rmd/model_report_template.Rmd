...
print(summary(robj$model$gam))
```

<br>
<br>
some plots on fitting
<br>
```{r, echo=FALSE, warning=FALSE, fig.width=10, fig.height=5, fig.align='left' }
...
vresp <- as.character(formula(robj$model$gam)[[2]])
...
if (length(vresp) == 2){
p1<- ggplot(robj$pred, aes(x = get(vfit), y = log(get(vresp[[2]])))) + geom_point() +
  ...
  labs(title = "Observed vs. fitted",
       ...
       y = paste0("log_", "bai (cm²)")
         ...
         p1<- ggplot(robj$pred, aes(x = get(vfit), y = get(vresp))) + geom_point() +
  geom_abline(slope = 1, intercept = 0, linewidth = 2 , color = "red", linetype = "dashed") +
  theme_minimal() +
  labs(title = "Observed vs. fitted",
...
# Create QQ plot
p2 <-ggplot(robj$pred, aes(sample = get(vres_norm))) +
  stat_qq() +                     # Add QQ points
  stat_qq_line(color = "red") +   # Add reference line
  labs(title = "QQ Plot of normalized residuals", x = "Theoretical Quantiles", y = "Sample Quantiles") +
  theme_minimal()
...
acf(robj$pred[[vres_norm]], plot = TRUE, main = "standardized residuals")
